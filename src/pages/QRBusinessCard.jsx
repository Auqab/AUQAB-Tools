...
function QRBusinessCard() {
  ...
  return (
    <>
      <SEO title="QR Business Card - AUQAB Tools" description="Create a QR code for your contact info." />
      <section className="tool-page">
        <div className="password-card">
          <h1>📇 QR Business Card</h1>
          <p className="tool-description">Enter your details and generate a vCard QR.</p>
          <input placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} className="url-input" />
          <input placeholder="Phone" value={phone} onChange={(e) => setPhone(e.target.value)} className="url-input" style={{ marginTop: 10 }} />
          <input placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="url-input" style={{ marginTop: 10 }} />
          {name && (
