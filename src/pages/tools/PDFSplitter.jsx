...
function PDFSplitter() {
  ...
  const splitPDF = async () => {
    ...
    try {
      ...
      alert("Split failed. Check the file and page ranges.");
    }
    setSplitting(false);
  };
  ...
  return (
    <>
      <SEO title="PDF Splitter - AUQAB Tools" description="Extract pages from a PDF file." />
      <section className="tool-page">
        <div className="password-card">
          <h1>✂️ PDF Splitter</h1>
          <p className="tool-description">Upload a PDF and specify page ranges to extract.</p>
          <input type="file" accept="application/pdf" onChange={handleFile} className="file-input" />
          <input
            type="text" placeholder="e.g. 1-3,5,7-9" value={ranges}
            onChange={(e) => setRanges(e.target.value)} className="url-input" style={{ margin: "15px 0" }}
          />
          <button className="generate" onClick={splitPDF} disabled={!file || splitting}>
